...
const CelebritySection = require("../models/celebratysection-model");
...
const getSectionDataBySectionId = async (req, res) => {
  try {
    ...
    // First find the celebrity section record
    const celebritySection = await CelebritySection.findById(id).populate('sectionmaster');
    
    if (!celebritySection) {
      return res.status(404).json({
        success: false,
        msg: "Celebrity Section not found",
      });
    }

    // Get the section master from the celebrity section
    const section = celebritySection.sectionmaster;
