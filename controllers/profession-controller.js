...
const syncCelebritySections = async (professionId, templateIds, specificCelebrityId = null) => {
  try {
    ...
    if (specificCelebrityId) {
      const celeb = await Celebraty.findById(specificCelebrityId).select("_id professionalIdentity");
      celebrities = celeb ? [celeb] : [];
    } else {
      celebrities = await Celebraty.find({
        "professionalIdentity.professions": professionId.toString(),
    ...
    for (const templateId of templateIds) {
      ...
      for (const celeb of celebrities) {
        ...
        // ✅ STEP 5: Celebrity.professionalIdentity.sections array mein push karo
        if (newSectionIds.length > 0) {
          await Celebraty.findByIdAndUpdate(celeb._id, {
            $addToSet: { "professionalIdentity.sections": { $each: newSectionIds } },
          });
...
const getSectionTemplateOptions = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Section templates fetched succccccccfessfully",
      data: templates,
    });
  } catch (error) {
    next(error);
